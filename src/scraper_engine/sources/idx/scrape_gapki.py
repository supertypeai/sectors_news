...
class GapkiScraper(SeleniumScraper):
        ...
        for item in soup.find_all('article', class_='post'):
            ...
            if link_element and time_element:
                ...
                if not all([title, source, timestamp_str]):
                    LOGGER.info("Skipping an article because it's missing crucial information.")
                    continue
                ...
                try:
                    ...
                    LOGGER.error(
                        f"Skipping article due to invalid date format: "
                        f"{timestamp_str} - {error}"
                    )

            else:
                LOGGER.info("[GAPKI.ID] Could not find link element and time element")
                continue
        
        LOGGER.info(f'total scraped source of gapki: {len(self.articles)}')
    ...
    def get_page(self, page_num):
        return (
            f'https://gapki.id/en/news/category/recent-news/'
            f'page/{page_num}/'
        )
