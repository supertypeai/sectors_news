...
class ICNScraper(Scraper):
    def extract_news(self, url):
        ...
        for item in soup.find_all('article'):
            ...
            if div:
                title = (
                    div.find('div', class_='elementor-post__text')
                    .find('h3', class_='elementor-post__title')
                    .find('a')
                    .text.strip()
                )
                source = div.find(
                ...
                timestamp = (
                    div.find('div', class_='elementor-post__meta-data')
                    .find('span', class_='elementor-post-date')
                    .text.strip()
                )
                timestamp = dateparser.parse(timestamp).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
        ...
        LOGGER.info(f'total scraped source of icn: {len(self.articles)}')
    ...
    def get_page(self, page_num):
        return (
            f'https://www.indonesiancoalandnickel.com/'
            f'lintasan-berita/page/{page_num}/'
        )
