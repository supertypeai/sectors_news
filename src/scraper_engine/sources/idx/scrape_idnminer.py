...
class IdnMinerScraper(Scraper):
        ...
        for item in soup.find_all('div', class_='col-12'):
            ...
            if link_tag and 'href' in link_tag.attrs:
                ...
                if '/news/detail/' in source:
                    ...
                    # timestamp tag
                    timestamp_container = link_tag.find(
                        'div',
                        class_='mb-2 text-muted small'
                    )
                    ...
                    if title_tag and timestamp_container:
                        ...
                        LOGGER.info(
                            "[IDNMINERSCRAPER] Could not find title or "
                            f"timestamp in a news item, link: {source}"
                        )

                else:
                    LOGGER.info(
                        f"[IDNMINERSCRAPER] Non-news link found: {source}"
                    )

            else:
                LOGGER.info(
                    "[IDNMINERSCRAPER] No '<a>' tag found in this item"
                )

        LOGGER.info(f'total scraped source of idnminer: {len(self.articles)}')
    ...
    def get_page(self, page_num):
        return (
            f'https://indonesiaminer.com/news?page={page_num}'
        )
